...
export class ChatMaestroPredictiveEngine {
  ...
  private generateWorkoutSuggestions(
    ...
    if (context.externalLifeVariables) {
      ...
      if (externalAnalysis.workImpact > 0.5) {
        suggestions.push({
          id: `work_conflict_suggestion_${Date.now()}`,
          type: 'workout_suggestion',
          confidence: 0.85,
          priority: 'high',
      ...
      if (externalAnalysis.stressImpact > 0.7) {
        suggestions.push({
          id: `high_stress_suggestion_${Date.now()}`,
          type: 'workout_suggestion',
          confidence: 0.9,
          priority: 'critical',
      ...
      if (externalAnalysis.scheduleConflicts.length > 0) {
        suggestions.push({
          id: `schedule_conflict_suggestion_${Date.now()}`,
          type: 'workout_suggestion',
          confidence: 0.8,
          priority: 'high',
      ...
      if (externalAnalysis.environmentalImpact > 0.5) {
        suggestions.push({
          id: `environmental_impact_suggestion_${Date.now()}`,
          type: 'workout_suggestion',
          confidence: 0.75,
          priority: 'medium',
      ...
      if (externalAnalysis.availabilityImpact > 0.7) {
        suggestions.push({
          id: `availability_suggestion_${Date.now()}`,
          type: 'workout_suggestion',
          confidence: 0.8,
          priority: 'medium',
  ...
  private generateLongTermPlanRecommendations(context: ChatContext): PredictiveRecommendation[] {
    ...
    } else if (userGoals.some(goal => goal.toLowerCase().includes('tono') || goal.toLowerCase().includes('tone'))) {
      primaryFocus = 'toning';
      secondaryFocuses.push('hypertrophy');
    } else if (userGoals.some(goal => goal.toLowerCase().includes('velocidad') || goal.toLowerCase().includes('speed'))) {
      primaryFocus = 'speed';
      secondaryFocuses.push('strength');
